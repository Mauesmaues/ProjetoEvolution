...
class GoogleAdsService {
    ...
    // Cliente usando Application Default Credentials (ADC)
    async getClient(customerAccountId) {
        ...
        try {
            // Configurar o caminho do Service Account
            ...
            console.log('[GoogleAdsService] Usando Service Account com ADC:', serviceAccountPath);

            // Definir a variável de ambiente para ADC
            process.env.GOOGLE_APPLICATION_CREDENTIALS = serviceAccountPath;

            // Método 1: Usar Application Default Credentials
            ...
            console.log('[GoogleAdsService] ✅ ADC autenticado com sucesso');

            // Obter access token do authClient
            const accessTokenResponse = await authClient.getAccessToken();
            const accessToken = accessTokenResponse.token;

            if (!accessToken) {
                throw new Error('Não foi possível obter access token via ADC');
            }

            console.log('[GoogleAdsService] ✅ Access token obtido via ADC');

            // Criar cliente Google Ads usando o access token
            const client = new GoogleAdsApi({
                developer_token: process.env.GOOGLE_DEVELOPER_TOKEN,
                // A biblioteca precisa desses parâmetros mesmo para Service Account
                client_id: process.env.GOOGLE_CLIENT_ID,
                client_secret: process.env.GOOGLE_CLIENT_SECRET,
                refresh_token: accessToken, // Usar access token como workaround
            });
            ...
            // Salva no cache por tempo menor (access tokens expiram em 1 hora)
            this.authCache.set(cacheKey, customer);
            this.cacheExpiry.set(cacheKey, Date.now() + this.CACHE_DURATION); // 30 minutos

            console.log('[GoogleAdsService] ✅ Cliente Google Ads criado com ADC');
            return customer;
